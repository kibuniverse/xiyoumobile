  ...
  return (
    <div className="graduate">
      <div className="yearContent">
        {yearslist.map((item) => <Link to={`${url}/${item.id}`} className={item.id === 2009 ? 'years' : 'year'} style={{ backgroundColor: state === item.index ? '#2e6acc' : 'white', color: state === item.index ? 'white' : '#2e6acc' }} onClick={(e) => { setState(item.index) }}><span>{item.id}</span></Link>)}
      </div>
